...
public class Client_thread extends Thread{
    ...
    /*
     * transmit_job(InetAddress master, String jobname)
     * This function transmit all files related to a job to the Crane
     * master.
     */
    public void transmit_job(InetAddress master, String jobname) throws IOException{
        File dir = new File(jobname);
        String[] entries = dir.list();

        Socket client_sock = new Socket(master, 5000);

        OutputStream os = client_sock.getOutputStream();
        DataOutputStream dos = new DataOutputStream(os);
        BufferedOutputStream bos = new BufferedOutputStream(os);
        byte[] mybytearray = new byte[5000];
        int num_file = entries.length;
        dos.writeUTF(jobname);
        dos.writeInt(num_file);
        for(String s: entries){
            File currentFile = new File(dir.getPath(),s);
            System.out.println("Filename: " + currentFile.getName());
            ...
            if(filename.contains(".db")){
                System.out.println("putting " + filename + " from " + dir.getPath() + "/" + currentFile.getName());
                Socket sdfs_sock = new Socket(InetAddress.getByName(get_own_ip()), 6000);
                OutputStream sdfsos = sdfs_sock.getOutputStream();
                DataOutputStream sdfsdos = new DataOutputStream(sdfsos);
                String cmd = "put " + dir.getPath() + "/" + currentFile.getName() + " " + filename;
                sdfsdos.writeUTF(cmd);
                sdfsos.close();
                sdfs_sock.close();
            }
        }
    ...
    public void run(){
        while(true){
            try{
                ...
                /* Used to submit Crane jobs to master */
                if(kbInput.split("\\s+")[0].equals("job")){
                    Clock clock = Clock.systemDefaultZone();
                    long start = clock.millis();
                    System.out.println(kbInput);
                    String dirname = kbInput.split("\\s+")[1];

                    if(member_list.contains(InetAddress.getByName(ip_array[0]))){
                        InetAddress master = InetAddress.getByName(ip_array[0]);
                        transmit_job(master, dirname);
                        InetAddress backup_master = InetAddress.getByName(ip_array[1]);
                        transmit_job(backup_master, dirname);
                    }
                    else{
                        InetAddress master = InetAddress.getByName(ip_array[1]);
                        transmit_job(master, dirname);
                    }

                    System.out.println("Job sent");
                    ServerSocket reply_sock = new ServerSocket(5000);
                    Socket clientSocket = reply_sock.accept();
                    DataInputStream dis = new DataInputStream(clientSocket.getInputStream());
                    String reply = dis.readUTF();
                    if(reply.equals("finished")){
                        long end = clock.millis();
                        System.out.println("Job finished: " + (end - start) + "ms");
                        reply_sock.close();
                    }
                    continue;
                }
